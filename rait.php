...
if (!empty($_POST)) {
    $d1 = $_POST['d1'];
    $d2 = $_POST['d2'];
    $esult = $link->query("select Kat,NamePO,Tiplis,vendor,sum(zakaz.kookop*tovar.Stomost) as objem from DOGOVOR, zakaz,TIPLISE,TIPPO,TOVAR,KATE where TOVAR.Nkat=KATE.NKat
 AND DOGOVOR.Ndog=TOVAR.Ndog 
and TOVAR.Ntov=zakaz.Ntov 
AND TIPLISE.Ntiplis=TOVAR.Ntiplis 
AND TIPPO.NtipPO=TOVAR.NtipPO 
and Dopl between '$d1' and '$d2'
 GROUP BY KATE.Kat, Tovar.NamePO,TIPLISE.Tiplis,Dogovor.Vendor ORDER BY kat ");
}
